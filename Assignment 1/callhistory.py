...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        # implemented already: Implement this method

        if isinstance(call, Call):

            month_key = call.time.month
            year_key = call.time.year

            self.outgoing_calls.setdefault((month_key, year_key),
                                           []).append(call)
    ...
    def register_incoming_call(self, call: Call) -> None:
        ...
        if isinstance(call, Call):
            ...
            self.incoming_calls.setdefault((month_key, year_key),
                                           []).append(call)
