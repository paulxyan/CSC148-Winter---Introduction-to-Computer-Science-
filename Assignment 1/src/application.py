...
def create_customers(log: dict[str, list[dict]]) -> list[Customer]:
    ...
    for cust in log['customers']:
        ...
        for line in cust['lines']:
            ...
            # Commenting for now for contracts
    ...
    for event_data in log['events']:
        ...
        new_billing_date = \
            datetime.datetime.strptime(event_data['time'],
                                       "%Y-%m-%d %H:%M:%S")
        new_billing_month = new_billing_date.month

        if billing_month != new_billing_month \
                or new_billing_date.year != billing_date.year:
            new_month(customer_list,
                      new_billing_date.month, new_billing_date.year)
            billing_date = new_billing_date
            billing_month = new_billing_month

        if event_data["type"] == "call":

            caller = event_data["src_number"]
            callee = event_data["dst_number"]
            time_called = new_billing_date
            # equivalent to the new_billing_date
            # Eventually get the time called

            total_seconds = event_data["duration"]

            source_location = (event_data["src_loc"][0],
                               event_data["src_loc"][1])
            destination_location = (event_data["dst_loc"][0],
                                    event_data["dst_loc"][1])

            new_call = Call(caller, callee, time_called,
                            total_seconds, source_location,
                            destination_location)

            # Registering the call for each customer call log

            receive = find_customer_by_number(caller, customer_list)

            receive.make_call(new_call)

            destination = find_customer_by_number(callee, customer_list)

            destination.receive_call(new_call)
