...
def mergesort3(lst: list) -> list:
    ...
    if len(lst) == 2:
        ...
        first_sort = mergesort3(lst[:one_third])
        second_sort = mergesort3(lst[one_third:2 * one_third])
        third_sort = mergesort3(lst[2 * one_third:])
...
def merge3(lst1: list, lst2: list, lst3: list) -> list:
    ...
    merged = merged + lst1[index1:] + lst2[index2:]

    merged2 = []

    index3 = 0
    index4 = 0
    while index3 < len(merged) and index4 < len(lst3):
        if merged[index3] <= lst3[index4]:
            merged2.append(merged[index3])
            index3 += 1
        else:
            merged2.append(lst3[index4])
            index4 += 1

    # Now either index1 == len(lst1) or index2 == len(lst2).
    assert index3 == len(merged) or index4 == len(lst3)
    # The remaining elements of the other list
    # can all be added to the end of <merged>.
    # Note that at most ONE of lst1[index1:] and lst2[index2:]
    # is non-empty, but to keep the code simple, we include both.

    return merged2 + merged[index3:] + lst3[index4:]
