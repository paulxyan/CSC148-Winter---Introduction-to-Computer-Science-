from assignments.a2.starter.huffman import HuffmanTree
...
def build_huffman_tree(freq_dict: dict[int, int]) -> HuffmanTree:
    ...
    def _update_merge(a: int, b: int, new_tree: HuffmanTree) -> None:
    ...
    _update_merge(pair[0], pair[1], current)

    while len(values) >= 2:
        sorted_vals = sorted(values)
        pair = (sorted_vals[0], sorted_vals[1])

        orig_check2 = orig.copy()

        if orig_check2.count(pair[0]) >= 1:
            orig_check2.remove(pair[0])

        # If both minimums come from original symbols, merge them.
        if pair[0] in orig and pair[1] in orig_check2:
            current = HuffmanTree(pair[0] + pair[1], HuffmanTree(pair[0]), HuffmanTree(pair[1]))
            _update_merge(pair[0], pair[1], current)
            continue
        ...
        tree_vals_check2 = tree_vals.copy()

        if pair[0] in tree_vals:
            tree_vals_check2.remove(pair[0])

        if (pair[0] in tree_vals and pair[1] in tree_vals_check2 and
